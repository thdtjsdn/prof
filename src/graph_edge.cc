...
void GraphEdge::Initialize() {
  ...
  edge_template_->SetAccessor(String::New("from"), GraphEdge::GetFromNode);
  edge_template_->SetAccessor(String::New("name"), GraphEdge::GetName);
  edge_template_->SetAccessor(String::New("to"), GraphEdge::GetToNode);
  edge_template_->SetAccessor(String::New("type"), GraphEdge::GetType);
  edge_template_->SetInternalFieldCount(1);
}

Handle<Value> GraphEdge::GetFromNode(Local<String> property, const AccessorInfo& info) {
  HandleScope scope;
...
Handle<Value> GraphEdge::GetToNode(Local<String> property, const AccessorInfo& info) {
  HandleScope scope;
  Local<Object> self = info.Holder();
